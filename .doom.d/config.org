...
Permanently visible bar showing all workspaces MRU-sorted left to right, with the active one
highlighted. Uses =tab-bar-mode= as a display layer for persp-mode workspaces. Works in terminal
mode (Emacs 28+ supports =tab-bar-lines= in TTY). Clicking a workspace switches to it.

Functions =my/workspace-names-mru-sorted=, =my/tab-bar-workspaces=, and =my/workspace-bar-refresh=
are defined in =+functions.el=.
...
The Emacs 31 face inheritance cycle fix is in =init.el= (must run before doom-themes loads).
...
Widen the name column (Doom default is 18) so buffer names aren't truncated.
...
DEPRECATED... using claude code only these days..
