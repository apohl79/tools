...
(defun projects--split-for-layout (layout)
  ...
  (pcase layout
    ...
    (4 (split-window-right)
       ...
       (other-window -1)
       (split-window-below))
    (6 (let* ((col1  (selected-window))
               (col-w (/ (window-total-width col1) 3))
               (col2  (split-window col1 col-w 'right))
               (col3  (split-window col2 col-w 'right)))
         (split-window col1 nil 'below)
         (split-window col2 nil 'below)
         (split-window col3 nil 'below)
         (select-window col1)))))
